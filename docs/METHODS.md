...
Topic classification is currently undergoing model-transition validation before the next full-corpus production run. The historical V4 classifier used GPT-5 mini, but September 2026 benchmarking of current GPT-5 mini, GPT-5.6 Luna and GPT-5 Nano showed materially different trade-offs in historical agreement, coding density and run-to-run reproducibility. GPT-5.6 Luna is presently the leading production candidate because it was the most internally consistent and comparatively restrained, but the final production model is not yet approved pending record-level human review of the 50-record benchmark sample. Full benchmark methods, results, runs and decision status are recorded in [TOPIC_CLASSIFIER_BENCHMARK_2026-09.md](TOPIC_CLASSIFIER_BENCHMARK_2026-09.md).

Whichever model is approved, topic classification uses the **OpenAI Responses API**, the fixed ontology, structured output constrained to permissible topic paths, and record-level checkpointing to permit reproducible resumption following interruption. Failed classifications and records explicitly flagged for review are retained outside the validated annotation set until resolved.
