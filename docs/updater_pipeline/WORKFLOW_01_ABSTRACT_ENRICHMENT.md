# Updater Pipeline — Workflow 01: abstract enrichment
...
Repair and enrich Lens records that have a DOI but no abstract, then emit canonical JSONL ready for Workflow 02 deduplication.

## Boundary

```text
Workflow 00: source retrieval
  -> Workflow 01: abstract repair / enrichment
  -> Workflow 02: deduplication
```

Workflow 01 does not deduplicate, screen, annotate, adjudicate, classify topics, merge into the master, or update the dashboard.
...
Workflow 02 treats a present `canonical` object as authoritative. Workflow 01 therefore writes a complete canonical bibliographic object rather than only adding `canonical.abstract`:
