...
"EXCLUDE records that are themselves standalone supplementary or additional material rather than the study itself, including supplementary files, additional files, supporting-information files, and MOESM records. Do not exclude the parent study merely because its metadata mentions or links to supplementary material.",
"EXCLUDE records that are themselves peer-review material, including peer-review reports, reviewer comments, author responses to reviewers, editorial decision letters, or equivalent peer-review-process records. Do not exclude the research article merely because peer-review material exists for it.",
...
PROMPT_VERSION <- "workflow04-multisource-v1-supplement-peer-review-exclusions"
...
payload <- function(r) {
  mapped <- r$mapped_fields %||% NULL
  if (is.list(mapped)) return(mapped)
  p <- (r$lens %||% list())$raw_payload %||% list()
...
work_id <- function(r) as.character(
  (r$reconciliation %||% list())$cluster_id %||%
  (r$identity %||% list())$work_id %||%
  (r$identity %||% list())$lens_id %||%
  canonical(r)$work_id %||%
  canonical(r)$lens_id %||%
  (r$reconciliation %||% list())$source_record_id %||%
  ""
)
...
if (!length(records)) stop("Workflow 02 representative input is empty")
if (!length(queue)) stop("Workflow 04 queue is empty")

ids <- vapply(records,work_id,character(1))
qids <- vapply(queue,work_id,character(1))
if(any(!nzchar(ids)) || anyDuplicated(ids)) stop("Workflow 02 work-ID invariant failed")
if(any(!nzchar(qids)) || anyDuplicated(qids)) stop("Queue work-ID invariant failed")
if(any(!qids %in% ids)) stop("Workflow 04 queue contains work IDs absent from Workflow 02 representatives")

# Only resolved Workflow 02 representatives may enter relevance screening.
for(r in queue) {
  rec <- r$reconciliation %||% list()
  status <- as.character(rec$cluster_status %||% "")
  if(!status %in% c("reconciled","singleton")) {
    stop(sprintf("Unresolved Workflow 02 cluster %s entered Workflow 04 queue",work_id(r)))
  }
  n <- r$notices %||% NULL
  if(!is.null(n) && !isTRUE(n$record_downstream_eligible %||% n$downstream_eligible %||% TRUE)) {
    stop(sprintf("Publication-blocked record %s entered Workflow 04 queue",work_id(r)))
  }
}
...
done_ids <- if(length(existing)) vapply(existing,function(x)as.character(x$work_id %||% ""),character(1)) else character()
if(anyDuplicated(done_ids)) stop("Existing Workflow 04 result IDs are duplicated")
if(any(!done_ids %in% vapply(queue,work_id,character(1)))) stop("Existing Workflow 04 results do not match current queue/sample")
...
rid <- vapply(all_results,function(x)as.character(x$work_id %||% ""),character(1))
if(anyDuplicated(rid) || setequal(rid,vapply(queue,work_id,character(1)))==FALSE) stop("Workflow 04 result identity invariant failed")
...
message("PASS: Multisource Workflow 04 R relevance-screening run complete with checkpointed per-work provenance.")
