...
read_jsonl <- function(path) {
  ...
  lapply(seq_along(lines), function(i) {
    tryCatch(
      ...
      error = function(e) stop(sprintf("Invalid canonical JSON at line %d: %s", i, conditionMessage(e)))
    )
  })
}
...
existing_abstract <- function(r) {
  for (v in list(r$canonical$abstract, payload(r)$abstract)) {
    if (!is.null(v) && length(v) && nzchar(trimws(as.character(v)))) return(as.character(v))
  }
  NULL
}
...
epmc_lookup <- function(d, max_attempts = 4L, waits = c(1, 2, 4)) {
    ...
    if (!inherits(resp, "error")) {
      ...
      if (status < 400L) {
        ...
        return(list(
          abstract = abstract,
          attempt = list(
            method = "europe_pmc_exact_doi",
            http_status = status,
            hit_count = dat$hitCount,
      ...
      retry_errors <- c(retry_errors, conditionMessage(resp))
    }

    if (attempt < max_attempts) {
      wait <- waits[min(attempt, length(waits))]
      message(sprintf("Europe PMC request for DOI %s failed on attempt %d (%s); retrying in %ss",
                      d, attempt, tail(retry_errors, 1), wait))
      Sys.sleep(wait)
    }
  }
  stop(sprintf("Europe PMC failed after %d attempts: %s", max_attempts, paste(retry_errors, collapse = " | ")))
}
...
if (any(vapply(rows, function(r) !is.null(r$deduplication) || !is.null(r$screening) || !is.null(r$screening_history), logical(1)))) {
  stop("Unexpected downstream state exists before Workflow 01")
}
...
provider_errors_path <- file.path(checkpoint_dir, "provider_errors.jsonl")
...
status_counts <- list()
provider_error_indices <- integer()

write_checkpoint_manifest <- function(processed, last_id, complete = FALSE, phase = "main_pass") {
  cp <- list(
    ...
    provider_errors_pending = length(provider_error_indices),
    complete = complete,
  ...
  message(sprintf("Workflow 01 checkpoint: phase=%s; %d/%d processed; last Lens ID=%s; provider errors pending=%d",
                  phase, processed, expected, last_id, length(provider_error_indices)))
}

make_enriched <- function(r, d, old, recovered, status, attempts) {
  ...
  enriched$abstract_enrichment <- list(
    ...
    status = status,
    doi = d,
    retrieved_at = if (!is.null(recovered)) now_utc() else NULL,
    attempts = attempts,
    cleaning = list(
      method = "html_jats_plaintext_v1_R",
      source_chars = nchar(source_abstract %||% "", type = "chars"),
      cleaned_chars = nchar(cleaned %||% "", type = "chars"),
      changed = !is.null(source_abstract) && !identical(source_abstract, cleaned)
    )
  )
  ...
  audit_row <- list(
    ...
    doi = d,
    status = status,
    abstract_chars = nchar(cleaned %||% "", type = "chars"),
    abstract_source_chars = nchar(source_abstract %||% "", type = "chars"),
    abstract_text_normalised = !is.null(source_abstract) && !identical(source_abstract, cleaned),
    attempts = attempts
  )
  list(record = enriched, audit = audit_row)
}
...
for (i in seq_along(rows)) {
  ...
  if (!is.null(old)) {
    ...
    if (inherits(result, "error")) {
      attempts <- list(list(method = "europe_pmc_exact_doi", outcome = "technical_error_deferred", error = conditionMessage(result)))
      status <- "technical_error_deferred"
      provider_error_indices <- c(provider_error_indices, i)
      message(sprintf("Workflow 01 deferred provider error at %d/%d, Lens ID %s, DOI %s: %s",
                      i, expected, lens_id(r), d, conditionMessage(result)))
    } else {
  ...
  built <- make_enriched(r, d, old, recovered, status, attempts)
  out[[i]] <- built$record
  audit[[i]] <- built$audit
  status_counts[[status]] <- (status_counts[[status]] %||% 0L) + 1L

  writeLines(toJSON(built$record, auto_unbox = TRUE, null = "null", na = "null", digits = NA), records_con)
  writeLines(toJSON(built$audit, auto_unbox = TRUE, null = "null", na = "null", digits = NA), audit_con)
  flush(records_con)
  flush(audit_con)

  if (i %% checkpoint_every == 0L || i == length(rows)) {
    write_checkpoint_manifest(i, lens_id(r), complete = FALSE, phase = "main_pass")
  }
}
...
if (!file.copy(checkpoint_audit_path, audit_path, overwrite = TRUE)) stop("Failed to copy checkpoint audit after main pass")
if (length(provider_error_indices)) {
  write_jsonl(lapply(provider_error_indices, function(i) audit[[i]]), provider_errors_path)
}

if (length(provider_error_indices)) {
  message(sprintf("Workflow 01 deferred retry phase: %d provider error(s) queued", length(provider_error_indices)))
  retry_waits <- c(30, 60, 120)
  for (round in seq_along(retry_waits)) {
    if (!length(provider_error_indices)) break
    message(sprintf("Workflow 01 deferred retry round %d: sleeping %ss before retrying %d record(s)",
                    round, retry_waits[[round]], length(provider_error_indices)))
    Sys.sleep(retry_waits[[round]])

    still_failed <- integer()
    for (i in provider_error_indices) {
      r <- rows[[i]]
      d <- doi(r)
      old <- existing_abstract(r)
      result <- tryCatch(epmc_lookup(d, max_attempts = 6L, waits = c(2, 4, 8, 16, 30)), error = identity)

      old_status <- audit[[i]]$status
      status_counts[[old_status]] <- max(0L, (status_counts[[old_status]] %||% 1L) - 1L)

      if (inherits(result, "error")) {
        attempts <- c(audit[[i]]$attempts, list(list(
          method = "europe_pmc_exact_doi_deferred_retry",
          round = round,
          outcome = "technical_error",
          error = conditionMessage(result)
        )))
        status <- "technical_error_deferred"
        built <- make_enriched(r, d, old, NULL, status, attempts)
        still_failed <- c(still_failed, i)
        message(sprintf("Workflow 01 deferred retry still failing: Lens ID %s, DOI %s", lens_id(r), d))
      } else {
        recovered <- result$abstract
        attempts <- c(audit[[i]]$attempts, list(result$attempt))
        status <- if (!is.null(recovered)) "abstract_recovered_after_retry" else "no_abstract_recovered_after_retry"
        built <- make_enriched(r, d, old, recovered, status, attempts)
        message(sprintf("Workflow 01 deferred retry resolved: Lens ID %s, DOI %s, status=%s", lens_id(r), d, status))
      }
    ...
    provider_error_indices <- still_failed
    write_jsonl(out, checkpoint_records_path)
    write_jsonl(audit, checkpoint_audit_path)
    if (length(provider_error_indices)) {
      write_jsonl(lapply(provider_error_indices, function(i) audit[[i]]), provider_errors_path)
    } else if (file.exists(provider_errors_path)) {
      file.remove(provider_errors_path)
    }
    write_checkpoint_manifest(expected, lens_id(rows[[expected]]), complete = FALSE, phase = sprintf("deferred_retry_round_%d", round))
  }
}
...
if (length(provider_error_indices)) {
  write_checkpoint_manifest(expected, lens_id(rows[[expected]]), complete = FALSE, phase = "blocked_provider_errors")
  stop(sprintf(
    "Workflow 01 blocked after deferred retries: %d Europe PMC request(s) remain technically unreachable. Checkpointed enriched records and audit are preserved in %s",
    length(provider_error_indices), checkpoint_dir
  ))
}
...
checkpoint_lines <- readLines(checkpoint_records_path, warn = FALSE, encoding = "UTF-8")
checkpoint_lines <- checkpoint_lines[nzchar(trimws(checkpoint_lines))]
if (length(checkpoint_lines) != expected) stop(sprintf("Checkpoint cardinality failure: expected %d records, found %d", expected, length(checkpoint_lines)))
...
report <- list(
  ...
  expected_records = expected,
  status_counts = status_counts,
  abstracts_recovered = (status_counts[["abstract_recovered"]] %||% 0L) + (status_counts[["abstract_recovered_after_retry"]] %||% 0L),
  abstract_texts_normalised = sum(vapply(audit, function(x) isTRUE(x$abstract_text_normalised), logical(1))),
  provider_errors_remaining = 0L,
  checkpoint_every = checkpoint_every,
...
manifest$abstract_enrichment <- list(
  workflow = "workflow_01_abstract_enrichment",
  implementation_language = "R",
...
write_checkpoint_manifest(expected, lens_id(rows[[expected]]), complete = TRUE, phase = "complete")
