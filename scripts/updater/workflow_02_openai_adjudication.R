...
message(sprintf('OpenAI duplicate adjudication: %d queued pairs; %d already checkpointed; %d reusable prior decisions; model=%s',length(queue),length(done_keys),length(prior_map),model))
...
for (i in seq_along(queue)) {
  ...
  result<-tryCatch({
    ...
    list(
      ...
      output_tokens=usage$output_tokens%||%NULL,
      total_tokens=usage$total_tokens%||%NULL,
      technical_failure=FALSE,error=NULL,
      adjudicated_at=now_utc(),queue_row=i,
      deterministic_basis=ca$basis%||%'',
      title_similarity=ca$title_similarity%||%NULL,
      abstract_similarity=ca$abstract_similarity%||%NULL
