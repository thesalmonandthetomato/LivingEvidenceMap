...
output_dir<-arg("--output-dir","outputs/workflow04_consensus_merged")
expected_shards<-as.integer(arg("--expected-shards","8"))
...
if(length(sum_files)!=expected_shards||length(layer_files)!=expected_shards){
  stop(sprintf("Expected %d shard summaries/layers; found %d/%d",expected_shards,length(sum_files),length(layer_files)),call.=FALSE)
}
summaries<-lapply(sum_files,fromJSON,simplifyVector=FALSE)
idx<-vapply(summaries,function(x)as.integer(x$shard_index),integer(1))
if(!setequal(idx,seq_len(expected_shards))||anyDuplicated(idx))stop("Shard-index coverage invariant failed",call.=FALSE)
if(any(vapply(summaries,function(x)as.integer(x$shard_count)!=expected_shards,logical(1))))stop("Shard-count invariant failed",call.=FALSE)
...
if(any(!dec%in%c("retain","exclude","uncertain")))stop("Invalid final decision in merged layer",call.=FALSE)
pv<-vapply(rows,function(x)scalar((x$screening %||% list())$prompt_sha256),character(1))
if(any(pv!=expected_sha))stop("Merged row prompt SHA mismatch",call.=FALSE)

ord<-order(ids)
rows<-rows[ord];ids<-ids[ord];dec<-dec[ord]
