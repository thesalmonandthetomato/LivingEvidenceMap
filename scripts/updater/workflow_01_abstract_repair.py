#!/usr/bin/env python3
"""Workflow 01: source-agnostic abstract repair/enrichment.

Inputs are preserved Workflow 00 outputs. Existing abstracts are never replaced.
Missing abstracts are repaired first from another preserved source by exact DOI,
...
def main():
    ...
    for src,rows in sources.items():
        ...
        for r in rows:
            ...
            if old:
                existing_n+=1
                meta={"workflow":"01","status":"existing_abstract","source_record":src,"doi":d,"repaired_at":None,"method":"source_native","canonical_store_modified":False}
                out.append(set_abstract(r,old,meta))
            elif d:
                donor=choose_donor(donors.get(d,[]),src)
                if donor:
                    repaired_by_cross+=1
                    meta={"workflow":"01","status":"abstract_repaired_cross_source","source_record":src,"doi":d,"repaired_at":now(),"method":"exact_doi_preserved_source","donor_source":donor["source"],"donor_record_id":donor["record_id"],"canonical_store_modified":False}
                    out.append(set_abstract(r,donor["abstract"],meta))
                else:
                    idx=len(out)
                    meta={"workflow":"01","status":"pending_external_repair","source_record":src,"doi":d,"repaired_at":None,"method":None,"canonical_store_modified":False}
                    out.append(set_abstract(r,None,meta))
                    unresolved_by_doi[d].append((src,idx))
            else:
                missing_no_doi+=1
                meta={"workflow":"01","status":"missing_no_doi","source_record":src,"doi":None,"repaired_at":None,"method":None,"canonical_store_modified":False}
                out.append(set_abstract(r,None,meta))
        prepared[src]=out
    ...
    if not args.no_external:
        ...
        for d,targets in unresolved_by_doi.items():
            ...
            for src,idx in targets:
                r=prepared[src][idx]
                if hit["abstract"]:
                    meta={"workflow":"01","status":"abstract_repaired_europe_pmc","source_record":src,"doi":d,"repaired_at":now(),"method":"europe_pmc_exact_doi","attempt":hit["attempt"],"canonical_store_modified":False}
                    ...
                    meta={"workflow":"01","status":status,"source_record":src,"doi":d,"repaired_at":None,"method":"europe_pmc_exact_doi","attempt":hit["attempt"],"canonical_store_modified":False}
    ...
    report={
        "workflow":"01_abstract_repair","status":"success","created_at":now(),
        "inputs":expected,"existing_abstract_records":existing_n,
        "cross_source_repairs":repaired_by_cross,"missing_without_doi":missing_no_doi,
