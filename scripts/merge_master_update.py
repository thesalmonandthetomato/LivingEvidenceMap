...
OUT=Path('data/reference/salmon_evidence_map_2026-08-14.csv')
...
def identity_key(r):
    lens=tv(r,['lens_id','Lens ID','LensID'])
    doi=tv(r,['doi','DOI','doi_id'])
    title=tv(r,['title','Title','document_title','Document Title']).lower()
...
master,mcols=read_csv(MASTER)
update,ucols=read_csv(UPDATE)
topics,trows,trecs=topic_map(TOPICS)
if not update: raise SystemExit('Update is empty')
master_original=len(master)
master,master_collapsed=collapse_exact_master_duplicates(master)
...
for i,r in enumerate(update,1):
    k=n(r.get('record_id')) or str(i)
    t=topics.get(k,{'ids':set(),'paths':set(),'review':False,'reasons':set()})
...
with MANIFEST.open('w',newline='',encoding='utf-8') as f:
    w=csv.writer(f); w.writerows([
        ['metric','value'],
        ['master_records_original',master_original],
        ['master_exact_duplicate_rows_removed',master_collapsed],
        ['master_records_after_exact_duplicate_cleanup',len(master)],
        ['update_records',len(update)],
        ['final_records',len(final)],
        ['topic_assignment_rows',trows],
        ['topic_records_with_output',trecs],
        ['within_update_duplicates_detected',0],
        ['update_vs_master_duplicates_detected',0],
        ['candidate_duplicate_identity_keys',0],
        ['source_master',str(MASTER)],
        ['source_update',str(UPDATE)],
        ['source_topics',str(TOPICS)],
    ])
print(f'Master records original: {master_original}')
print(f'Exact duplicate master rows removed: {master_collapsed}')
