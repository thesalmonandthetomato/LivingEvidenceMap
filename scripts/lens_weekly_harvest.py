...
while True:
    ...
    if not scroll_id:
        raise SystemExit("Lens API returned more records but no scroll_id")
    payload = {"scroll_id": scroll_id, "scroll": SCROLL, "include": include}

def norm(v):
    return "" if v is None else re.sub(r"\s+", " ", str(v).strip().lower())

def key(rec):
    lens = norm(rec.get("lens_id"))
...
manifest = {
    ...
    "publication_type_exclusions": cfg.get("publication_type_exclusions", []),
