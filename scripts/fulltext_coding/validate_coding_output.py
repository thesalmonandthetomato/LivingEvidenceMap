...
This validator checks only machine-checkable structure and bookkeeping. It does not
judge scientific correctness, whether a document should be considered complete, or
whether a substantive field ought to have been coded.
"""
...
SUBSTANTIVE_FIELDS = {
    "document_type", "review_type", "study_type", "study_design", "research_approach", "setting", "sample_size", "sample_unit",
    "study_period", "location_region", "location_country", "species", "other_farmed_species", "study_population", "aquaculture_facility",
    "system_studied", "production_stage", "fish_life_stage", "impact_type", "impact_details", "outcome_measured",
    "exposure_intervention", "comparator", "methodology_for_data_collection", "funding_body", "funder", "research_question",
    "objectives_summary", "ontology_codes", "multiple_studies_flag", "multiple_studies_reason"
}
ARRAY_FIELDS = {
    "study_design", "setting", "sample_unit", "species", "other_farmed_species", "aquaculture_facility", "production_stage",
    "fish_life_stage", "impact_type", "outcome_measured", "methodology_for_data_collection", "ontology_codes",
    "non_methods_results_evidence_fields", "not_reported_fields", "evidence"
}
STRING_FIELDS = {
    "study_period", "location_region", "location_country", "study_population", "system_studied", "impact_details", "exposure_intervention",
    "comparator", "funding_body", "funder", "research_question", "objectives_summary", "multiple_studies_reason", "document_completeness_evidence"
}
...
def validate(record: dict) -> list[str]:
    ...
    elif any(k not in record["run_metadata"] for k in ("schema_version", "ontology_version", "model", "provider", "timestamp_utc")):
        missing_meta = [k for k in ("schema_version", "ontology_version", "model", "provider", "timestamp_utc") if k not in record["run_metadata"]]
        errors.append(f"run_metadata missing {missing_meta}")
    ...
    # Type checks apply to both complete and explicitly incomplete records.
    ...
    for field in STRING_FIELDS:
        if record.get(field) is not None and not isinstance(record.get(field), str): errors.append(f"{field} must be string or null")
    ...
    incomplete = record.get("document_type") is None
    if incomplete:
        ...
        if not record.get("species") or any(v not in SPECIES for v in record["species"]): errors.append("species must be a non-empty array containing only closed-vocabulary species")
    ...
    if not isinstance(fc, dict):
        ...
        if any(v == "FOUND" for v in fc.values()): errors.append("FOUND is prohibited in field_completeness")
        ...
        for field, status in fc.items():
            ...
            elif status == "NOT FOUND" and not (value is None or value == [] or _field_is_inapplicable(field, value)):
                errors.append(f"field_completeness says NOT FOUND but {field} is populated")
            elif status is None and not _field_is_inapplicable(field, value):
