...
CURRENT_FIELDS = {
    ...
    "exposure_intervention", "comparator", "outcome_measured", "impact_type", "impact_details", "funding_body",
...
def validate(record: dict) -> list[str]:
    ...
    if missing:
        errors.append(f"missing current extraction fields: {missing}")
    ...
    if record.get("document_type") == "review" and record.get("review_type") == "not_applicable":
        errors.append("review requires a review_type")
    if record.get("document_type") != "review" and record.get("review_type") != "not_applicable":
        errors.append("review_type must be not_applicable unless document_type is review")
    ...
    if not isinstance(record.get("study_design"), list) or any(v not in STUDY_DESIGNS for v in record.get("study_design", [])):
        errors.append("study_design must be an array containing only controlled values")
    ...
    for field in ("setting", "species", "other_farmed_species", "sample_unit", "aquaculture_facility", "production_stage", "fish_life_stage", "impact_type", "outcome_measured", "ontology_codes", "not_reported_fields", "evidence"):
        if not isinstance(record.get(field), list):
            errors.append(f"{field} must be a JSON array")

    if not isinstance(record.get("species"), list) or not record["species"]:
        errors.append("species must be a non-empty array")
    ...
    if record.get("multiple_studies_flag"):
        if not isinstance(record.get("multiple_studies_reason"), str) or not record["multiple_studies_reason"].strip():
            errors.append("multiple_studies_flag=true requires multiple_studies_reason")
    elif record.get("multiple_studies_reason") is not None:
        errors.append("multiple_studies_reason must be null when multiple_studies_flag=false")
    ...
    if not isinstance(fc, dict):
        ...
        if expected_nr != actual_nr:
            errors.append(f"not_reported_fields mismatch: expected {expected_nr}, got {actual_nr}")
    ...
    if isinstance(record.get("evidence"), list):
        ...
        for i, item in enumerate(record["evidence"]):
            ...
            if item.get("field") not in CURRENT_FIELDS:
                errors.append(f"evidence[{i}] references non-current field {item.get('field')!r}")
            if not isinstance(item.get("text"), str) or not item.get("text", "").strip():
                errors.append(f"evidence[{i}] requires article-text evidence")
        for field in CURRENT_FIELDS:
            value = record.get(field)
            # Explicitly inapplicable values do not require evidence; populated substantive values do.
            ...
            if field not in evidence_fields:
                errors.append(f"populated current field lacks field-specific evidence: {field}")

    if "sample_size" in record or "methodology_for_data_collection" in record or "funder" in record or "intervention" in record or "exposure" in record:
        errors.append("legacy extraction field present; remove it")
    return errors
