...
def validate(record: dict) -> list[str]:
    ...
    if record.get("document_type") is None:
        ...
        if not isinstance(record.get("document_completeness_evidence"), str) or not record["document_completeness_evidence"].strip(): errors.append("incomplete-document record requires document_completeness_evidence")
        return errors
    ...
    elif record.get("non_methods_results_evidence") and not record["non_methods_results_evidence_fields"]: errors.append("non_methods_results_evidence=true requires at least one affected field")
