...
SCHEMA_KEYS = {
    ...
    "system_studied", "production_stage", "farming_system", "impact_type", "impact_details", "outcome_measured",
    "exposure_pathway", "mitigation_evaluation", "intervention", "exposure", "comparator",
...
STUDY_TYPES = {"experimental", "observational", "modelling", "not_stated"}
STUDY_DESIGNS = {"BA", "CI", "BACI", "RCT", "Time-series", "Modelling", "Qualitative", "not_stated"}
...
def validate(record: dict) -> list[str]:
    ...
    for field in ("setting", "species", "aquaculture_facility", "production_stage", "farming_system", "impact_type", "outcome_measured", "exposure_pathway", "methodology_for_data_collection", "ontology_codes", "evidence"):
        if field in record and not isinstance(record[field], list): errors.append(f"{field} must be a JSON array")
    if "aquaculture_facility" in record:
        bad={"wild", "mixed"}.intersection(set(record["aquaculture_facility"]))
        if bad: errors.append(f"aquaculture_facility cannot contain: {sorted(bad)}")
    if isinstance(record.get("setting"), list) and "mixed" in record["setting"]: errors.append("setting must not contain 'mixed'; use multiple values instead")
