...
STRING_FIELDS = {
    "study_period", "location_region", "location_country", "study_population", "system_studied", "impact_details",
    "exposure_intervention", "comparator", "funding_body", "funder", "research_question", "objectives_summary",
    "multiple_studies_reason", "document_completeness_evidence"
}


def _field_is_inapplicable(field: str, value) -> bool:
    """Return True only for schema-valid representations of an inapplicable field."""
    if field in {"review_type", "study_type", "research_approach"}:
        return value == "not_applicable"
    if field == "study_design":
        return isinstance(value, list) and value == ["not_applicable"]
    if field in {"setting", "sample_unit", "other_farmed_species", "aquaculture_facility", "production_stage", "fish_life_stage", "impact_type", "outcome_measured", "methodology_for_data_collection", "ontology_codes"}:
        return isinstance(value, list) and value == []
    if field in {"sample_size", "study_period", "location_region", "location_country", "study_population", "system_studied", "impact_details", "exposure_intervention", "comparator", "funding_body", "funder", "research_question", "objectives_summary", "multiple_studies_reason"}:
        return value is None
    if field == "multiple_studies_flag":
...
def validate(record: dict) -> list[str]:
    ...
    if "mitigation_evaluation" in record:
        errors.append("legacy field mitigation_evaluation is prohibited")
    if "intervention" in record or "exposure" in record:
        errors.append("legacy separate intervention/exposure fields are prohibited; use exposure_intervention")

    # An incomplete-document state is a model coding decision, not a validator judgement.
    # The validator only checks that the required evidence is present and that the output
    # retains the required workflow metadata.
    ...
    if record.get("doi") is not None and not isinstance(record.get("doi"), str):
        errors.append("doi must be string or null")
    if record.get("openalex_id") is not None and not isinstance(record.get("openalex_id"), str):
        errors.append("openalex_id must be string or null")
    if record.get("title") is not None and not isinstance(record.get("title"), str):
        errors.append("title must be string or null")
    if record.get("year") is not None and not isinstance(record.get("year"), int):
    ...
    if any(v not in STUDY_DESIGNS for v in record["study_design"]):
        errors.append("invalid study_design value(s)")
    if any(v not in SETTINGS for v in record["setting"]):
        errors.append("invalid setting value(s)")
    if any(v not in PRODUCTION_STAGES for v in record["production_stage"]):
        errors.append("invalid production_stage value(s)")
    if any(v not in FISH_LIFE_STAGES for v in record["fish_life_stage"]):
        errors.append("invalid fish_life_stage value(s)")
    if any(v not in AQUACULTURE_FACILITIES for v in record["aquaculture_facility"]):
        errors.append("invalid aquaculture_facility value(s)")
    if not all(isinstance(v, str) and v.strip() for v in record["sample_unit"]):
        errors.append("sample_unit must contain non-empty strings")
    if not all(isinstance(v, str) and v.strip() for v in record["other_farmed_species"]):
        errors.append("other_farmed_species must contain non-empty strings")
    if not isinstance(record.get("multiple_studies_flag"), bool):
        errors.append("multiple_studies_flag must be boolean")
    if not isinstance(record.get("non_methods_results_evidence"), bool):
        errors.append("non_methods_results_evidence must be boolean")
    if not all(isinstance(v, str) and v.strip() for v in record["not_reported_fields"]):
        errors.append("not_reported_fields must contain non-empty strings")
    ...
    if not isinstance(fc, dict):
        ...
        for field, status in fc.items():
            ...
            value = record[field]
            if status == "NOT FOUND":
                if not _field_is_inapplicable(field, value) and not (value is None or value == []):
                    errors.append(f"field_completeness says NOT FOUND but {field} is populated")
            elif status is None:
                if not _field_is_inapplicable(field, value):
                    errors.append(f"field_completeness says null/inapplicable but {field} is not represented as inapplicable")
