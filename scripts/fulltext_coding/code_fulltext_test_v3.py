#!/usr/bin/env python3
"""Full-text coding runner using the synchronized v3 prompt/schema/validator contract."""
...
def failed_checkpoint(path, raw, status, error):
    ts=datetime.now(timezone.utc).isoformat(); raw_file=path.parent/(path.stem+".raw_response.json")
    raw_file.write_text(json.dumps(raw,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    status.write_text(json.dumps({"source_prepared_file":path.name,"raw_response_file":raw_file.name,"status":"needs_review_or_retry","error_type":"invalid_json_or_structure","error":str(error),"checkpoint_rule":"raw_response_before_parsing","timestamp_utc":ts},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--input-dir",type=Path,required=True); ap.add_argument("--output-dir",type=Path,required=True); ap.add_argument("--ontology",type=Path,required=True); ap.add_argument("--schema",type=Path,required=True); ap.add_argument("--validator",type=Path,required=True); ap.add_argument("--prompt",type=Path,required=True); ap.add_argument("--model",default=DEFAULT_MODEL); ap.add_argument("--max-papers",type=int,default=10); args=ap.parse_args()
    ...
    for i,path in enumerate(files,1):
        ...
        prepared=load(path)
        user=("Code this article according to the supplied schema and ontology. Return a single valid JSON object only. Return exactly the current extraction fields and the explicitly permitted audit/evidence structures. Do not return schema descriptions, provenance metadata, legacy fields, or explanatory top-level keys. The substantive annotation fields must be at the TOP LEVEL.\n\nCODING SCHEMA:\n"+json.dumps(schema,ensure_ascii=False,indent=2)+"\n\nONTOLOGY CSV:\n"+ontology+"\n\nPREPARED ARTICLE:\n"+json.dumps(prepared,ensure_ascii=False))
        ...
        for attempt in range(4):
            try:
                ...
                try: annotation=parse_annotation(text)
                    ...
                    failed_checkpoint(path,data,status_out,exc); print(f"  INVALID JSON/STRUCTURE for {path.name}; raw response checkpointed; continuing batch",flush=True); break
                ...
                status_out.write_text(json.dumps({"source_prepared_file":path.name,"annotation_file":out.name,"raw_response_file":raw_out.name,"status":"generated","validation_warning_count":len(warnings),"checkpoint_rule":"checkpoint_before_validation","timestamp_utc":datetime.now(timezone.utc).isoformat()},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
                print(f"  CHECKPOINTED {out.name} with validation warnings before downstream processing",flush=True); break
            except Exception as exc:
                print(f"  attempt {attempt+1}/4 failed before successful response: {exc}",flush=True)
                if attempt==3:
                    status_out.write_text(json.dumps({"source_prepared_file":path.name,"status":"api_or_execution_error","error":str(exc),"timestamp_utc":datetime.now(timezone.utc).isoformat()},ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(f"  PAPER FAILED, continuing batch: {path.name}",flush=True); break
