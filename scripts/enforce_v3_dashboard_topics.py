        ...
        try:
            ...
            if isinstance(obj, list):
                if all(isinstance(x, list) for x in obj):
                    return [[clean(v) for v in x if clean(v)] for x in obj]
...
with MASTER.open(newline='', encoding='utf-8-sig') as fh:
    ...
    if not topic_field:
        raise SystemExit('Master has no authoritative topic_hierarchy_paths column')
    master_topics = {}
    for row in reader:
        master_topics[str(row.get('record_id', '')).strip()] = parse_paths(row.get(topic_field, ''))
...
for record in d.get('records', []):
    ...
    raw_paths = master_topics.get(rid, [])
    valid_paths = []
    for path in raw_paths:
        ...
        if t in prefixes:
            ...
            invalid.append((rid, ' > '.join(path)))
    valid_paths = list(dict.fromkeys(tuple(p) for p in valid_paths))
    new_topics = list(dict.fromkeys(part for path in valid_paths for part in path))
    new_paths = [list(p) for p in valid_paths]
    if record.get('topic_paths') != new_paths or record.get('topics') != new_topics:
        changed += 1
    record['topic_paths'] = new_paths
    record['topics'] = new_topics
    for key in list(record):
        if key.startswith('topic_level_'):
            del record[key]
    for i in range(1, max((len(p) for p in new_paths), default=0) + 1):
        vals = list(dict.fromkeys(p[i - 1] for p in new_paths if len(p) >= i))
        if vals:
            record[f'topic_level_{i}'] = vals
...
if invalid:
    print(f'Ignored {len(invalid):,} non-V3 paths from the master topic field.')
