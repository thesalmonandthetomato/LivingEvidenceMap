...
ALIASES = {
    ...
    'journal': ['journal','journal_title','source_title','publication_name','container_title','publication'],
...
def lens_url(v):
    ...
    if s.startswith('http://') or s.startswith('https://'):
        return s
    # Only construct a Lens URL from an explicit Lens document identifier.
    if s.lower().startswith('lens-'):
        return 'https://www.lens.org/lens/scholar/' + s
    return ''

if not MASTER.exists() or not DASH.exists():
    raise SystemExit('Required dashboard/master file missing')

with MASTER.open(newline='', encoding='utf-8-sig') as f:
    reader = csv.DictReader(f)
    fields = reader.fieldnames or []
    rows = reader
    cols = {k: pick(fields, v) for k, v in ALIASES.items()}
    by_id = {}
    for row in rows:
        rid = clean(row.get(cols['id'], '')) if cols['id'] else ''
        if not rid: continue
        by_id[rid] = {
            'authors': clean(row.get(cols['authors'], '')) if cols['authors'] else '',
            'journal': clean(row.get(cols['journal'], '')) if cols['journal'] else '',
            'volume': clean(row.get(cols['volume'], '')) if cols['volume'] else '',
            'pages': clean(row.get(cols['pages'], '')) if cols['pages'] else '',
            'doi': normalise_doi(row.get(cols['doi'], '')) if cols['doi'] else '',
            'lens_url': lens_url(row.get(cols['lens'], '')) if cols['lens'] else '',
        }

payload = json.loads(DASH.read_text(encoding='utf-8'))
for rec in payload.get('records', []):
    meta = by_id.get(clean(rec.get('record_id', '')), {})
    for key in ('authors','journal','volume','pages','lens_url'):
        rec[key] = meta.get(key, '')
    # Keep DOI authoritative from the dashboard builder if present, otherwise use master metadata.
