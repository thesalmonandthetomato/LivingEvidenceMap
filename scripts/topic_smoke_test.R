...
# Intentionally one API call only. Full topic classification remains the final
# production stage and is not rerun for the current Lens refresh.
...
if (!nzchar(Sys.getenv("OPENAI_API_KEY"))) stop("OPENAI_API_KEY is required for the topic smoke test.")

result <- classify_topic_hierarchy_single_call(title = title, abstract = abstract,
                                                ontology = smoke_ontology, model = "gpt-5-mini")
