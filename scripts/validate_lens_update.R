...
# Purpose: Validate a Lens search-update file before downstream processing.
# =============================================================================

validate_lens_update <- function(path, min_records = 1L) {
  if (!fs::file_exists(path)) stop("Lens update file does not exist: ", path, call. = FALSE)
  info <- fs::file_info(path)
  if (is.na(info$size) || info$size <= 0) stop("Lens update file is empty: ", path, call. = FALSE)
  if (tolower(tools::file_ext(path)) != "ris") stop("Expected a Lens RIS file (.ris).", call. = FALSE)
  ...
  ids <- grep("^ID[[:space:]]*-[[:space:]]*", raw, ignore.case = TRUE)
