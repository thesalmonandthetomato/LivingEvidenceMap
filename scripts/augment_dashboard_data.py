    ...
    if paths:
        ...
        if levels:
            ...
            for i,vals in levels.items():
                names=set(str(x).strip() for x in vals if str(x).strip())
                for x in names: level_counts[str(i)][x]+=1
                for s in r['species']:
                    for x in names: species_level[str(i)][(s,x)]+=1
            terminal.update(set(str(x).strip() for x in levels[str(deepest)] if str(x).strip()))
