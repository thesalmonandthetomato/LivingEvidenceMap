...
#
# The detector is deliberately target-independent: callers provide the records
# and gazetteer explicitly. Matches are case-insensitive by default, constrained
# to token boundaries, and overlapping matches are reduced to the longest span.
...
detect_geography_mentions <- function(records, gazetteer) {
  ...
  if (anyDuplicated(records$record_id) || anyDuplicated(records$record_sequence)) {
    stop("Records must contain unique record_id and record_sequence values.", call. = FALSE)
  }
  ...
  result <- do.call(rbind, lapply(seq_len(nrow(records)), function(i) {
    rbind(
