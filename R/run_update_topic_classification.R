...
ontology <- ontology %>% mutate(cue_terms = coalesce(supporting_terms_from_old_ontology, ""), prompt_line = paste0(hierarchy_path, " [", path_id, "] | Semantic cues: ", cue_terms))
valid_paths <- ontology$hierarchy_path

classify_one <- function(title, abstract) {
  system_prompt <- paste("You are classifying scientific abstracts for a systematic evidence map of farmed salmon and rainbow trout research.", "Select every valid three-level hierarchy path listed below that represents a substantive objective, intervention, exposure, measured outcome, interpretation or application of the study.", "A path need not be the primary focus to be substantive.", "Semantic cues are non-exhaustive and do not require literal matching.", "Do not assign a path from an isolated word, background mention, or motivation alone.", "Use only the listed hierarchy paths. Multiple paths are allowed when genuinely applicable.", "Return an empty assignments array only when no listed path applies.", sep = "\n")
  schema <- list(type = "object", properties = list(assignments = list(type = "array", items = list(type = "string", enum = I(valid_paths))), review_required = list(type = "boolean"), review_reason = list(type = c("string", "null"))), required = c("assignments", "review_required", "review_reason"), additionalProperties = FALSE)
  ...
  body <- list(model = "gpt-5-mini", store = FALSE, input = list(list(role = "system", content = list(list(type = "input_text", text = system_prompt))), list(role = "user", content = list(list(type = "input_text", text = user_prompt)))), text = list(format = list(type = "json_schema", name = "topic_hierarchy_classification", strict = TRUE, schema = schema)))
