...
source_json <- if (length(args) >= 1) args[[1]] else "data/master/current/living_evidence_map_master.json"
out_csv <- if (length(args) >= 2) args[[2]] else "docs/living_evidence_map_static.csv"
out_json <- if (length(args) >= 3) args[[3]] else "docs/dashboard-v07.json"
...
if (!file.exists(source_json)) stopf("Canonical JSON not found: %s", source_json)
...
raw <- fromJSON(source_json, simplifyVector = FALSE)
records <- if (is.list(raw) && !is.null(raw$records)) raw$records else raw
if (!is.list(records) || !length(records)) stopf("Canonical JSON contains no records")
...
write_json(payload, out_json, auto_unbox=TRUE, null="null", na="null", pretty=FALSE)
...
write_json(manifest, sub("\\.json$", "-manifest.json", out_json), auto_unbox=TRUE, pretty=TRUE)

cat(sprintf("Workflow 07 build complete: %d records; CSV=%s; dashboard JSON=%s\n", nrow(csv), out_csv, out_json))
