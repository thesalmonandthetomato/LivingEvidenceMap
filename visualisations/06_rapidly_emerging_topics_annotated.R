...
# background evidence-base growth. Adds n= labels at the final observed point
# of each topic.
...
master <- master %>% transmute(record_id = as.character(record_id), publication_year = suppressWarnings(as.integer(year)), topic_hierarchy_paths = as.character(topic_hierarchy_paths)) %>% filter(!is.na(publication_year))
...
expanded <- master %>% mutate(path = str_split(topic_hierarchy_paths, "\\s*;\\s*")) %>% unnest(path) %>% mutate(path = str_squish(path)) %>% filter(path != "") %>% distinct(record_id, publication_year, path)
...
# Identify the latest year with at least one tagged record for each topic and
# annotate that point with the actual number of records. A one-year x margin
# is reserved so labels remain inside the plotting panel.
final_points <- plot_data %>% filter(topic_records > 0) %>% group_by(topic) %>% filter(publication_year == max(publication_year)) %>% ungroup() %>% mutate(topic_plot = factor(topic, levels = topic_order, labels = unname(topic_labels[topic_order])), annotation_x = publication_year + 0.35, annotation_label = paste0("n = ", comma(topic_records)))
plot_data <- plot_data %>% mutate(topic_plot = factor(topic, levels = topic_order, labels = unname(topic_labels[topic_order])))
x_max_plot <- max(all_years) + 1
...
p <- ggplot(plot_data, aes(x = publication_year)) +
  ...
  geom_text(data = final_points, aes(x = annotation_x, y = topic_growth_index, label = annotation_label), inherit.aes = FALSE, hjust = 0, vjust = 0.5, colour = "#29434A", fontface = "bold", size = 2.8) +
  facet_wrap(~ topic_plot, ncol = 3, scales = "free_y") +
  scale_x_continuous(limits = c(2010, x_max_plot), breaks = c(2010, 2015, 2020, 2025), labels = c("2010", "2015", "2020", "2025"), expand = expansion(mult = c(0.01, 0))) +
  scale_y_continuous(labels = comma_format(accuracy = 1), expand = expansion(mult = c(0, 0.10))) +
  labs(title = "Rapidly emerging topics in the evidence base", subtitle = paste0("Growth indexed to 2010–2014; dashed grey = fitted background database growth (", number(background_growth_ratio, accuracy = 0.01), "× by 2021–2025)"), x = "Publication year", y = "Growth index (2010–2014 mean = 100)", caption = "Red = topic output; dashed grey = background evidence-base trend. Red above grey indicates faster-than-background growth. Labels show the number of records tagged with each topic in its final observed year. Plastics and solid waste is indexed to its first observed year because it has no 2010–2014 records.") +
  theme_minimal(base_size = 10.5) +
  theme(panel.grid.major.y = element_line(colour = "#E5E8E9", linewidth = 0.3), panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), strip.background = element_rect(fill = "#EEF2F2", colour = NA), strip.text = element_text(face = "bold", colour = "#29434A", size = 8.0, lineheight = 0.9, hjust = 0.5), strip.placement = "inside", strip.clip = "on", axis.title = element_text(face = "bold", colour = "#45616A"), axis.text = element_text(colour = "#29434A", size = 8.2), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 8), plot.title = element_text(face = "bold", size = 17, colour = "#29434A"), plot.subtitle = element_text(colour = "#45616A", size = 10.5), plot.caption = element_text(colour = "#5B6D72", size = 8.2, hjust = 0), panel.spacing = grid::unit(1.25, "lines"), plot.background = element_rect(fill = "white", colour = NA), panel.background = element_rect(fill = "white", colour = NA), plot.margin = margin(16, 20, 18, 16))
